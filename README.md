...
> 首次启动会自动创建示例题库数据库（默认 `.obsidian/quiznow/题库.json`，位于
...
3. 生成过程中会显示"正在生成试卷…"提示，完成后弹出预览窗口：
   - **立即答题** → 直接开始考试；
   - **加入题库** → 全部题目写入题库数据库（默认 `.obsidian/quiznow/题库.json`）。
...
题库保存在**单个 JSON 数据库文件**中（默认 `.obsidian/quiznow/题库.json`，
...
**Q：数据保存在哪里？卸载插件会丢失吗？**
A：数据保存在插件 `data.json` 与题库数据库文件（默认 `.obsidian/quiznow/题库.json`，
